...
//        }
   ...
   List<ProdApplications> prodApplicationses = entityManager.createQuery("select pa from ProdApplications pa left join pa.reviewInfos ri where pa.regState in (:regStates) " +
	...
	public List<ProdApplications> fetchOnScreening() {
		...
		regStates.add(RegState.SAVED);
		regStates.add(RegState.NEW_APPL);
		regStates.add(RegState.REGISTERED);
		
		List<ProdApplications> prodApplicationses = entityManager.createQuery("select pa from ProdApplications pa where pa.regState not in (:regStates) " +
				"and pa.moderator is null")              
                .setParameter("regStates", regStates)
                .getResultList();
        return prodApplicationses;
	}
	...
	public List<ProdApplications> fetchAwaitingReview() {
		...
		List<ProdApplications> prodApplicationses = entityManager.createQuery("select pa from ProdApplications pa where " +
				"pa.moderator is not null and pa.reviewInfos is empty") 				
                .getResultList();
        return prodApplicationses;
	}
	...
	public List<ProdApplications> fetchOnReview() {
		...
		List<ProdApplications> prodApplicationses = entityManager.createQuery("select pa from ProdApplications pa where pa.regState in (:regStates) " +
				"and pa.reviewInfos is not empty") 
				.setParameter("regStates", regStates)
                .getResultList();
        return prodApplicationses;
	}
     ...
     */
	public List<ProdApplications> findProdApplByApplicants(Long appIDs, String year){
		
		String sql = "select pa.id from prodapplications pa " +				
				"join product as p on p.id=pa.PROD_ID " +
				"join applicant as a on a.applcntId=pa.APP_ID " +
				"left join dosform as df on df.uid=p.DOSFORM_ID " +
				...
				"where year(pa.regExpiryDate)=" +year +" "+
				"and pa.regState like '" + RegState.REGISTERED + "' " +			
				"and a.applcntId =" +appIDs +" ";
				
		List<BigInteger> prodApp = entityManager.createNativeQuery(sql).getResultList();
		List<ProdApplications> prodAppls = new ArrayList<ProdApplications>();//entityManager.createQuery(sql).getResultList();
		for (BigInteger prodApplId : prodApp) {
			ProdApplications pa = findProdApplications(prodApplId.longValue());
			prodAppls.add(pa);			
		}
		return prodAppls;
		//return prodApplicationsDAO.findProdApplicationsByApplicants(aplicant_id, category, year);
	}
}
