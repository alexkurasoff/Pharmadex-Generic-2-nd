...
//        StreamedContent download = new DefaultStreamedContent(ist, "image/jpg", "After3.jpg");
   ...
   /**
    * create timeLine, acknowledgement letter (TemplateType - DOSSIER_RECIEPT)
    * @param prodAppID
    * @return 
    */
    public String submitApp(Long prodAppID) {
    	    	
        context = FacesContext.getCurrentInstance();
        bundle = context.getApplication().getResourceBundle(context, "msgs");
//moved to ProdApplicationServiceMZ AK 2017-09-08
/*		String no = prodApplications.getProdSrcNo();
		ProdApplicationsService paser = prodApplicationsServiceMZ.getProdApplicationsService();
		if(no == null || no.trim().equals("")){  
			if(prodApplications.getProdAppNo()==null){
				prodApplications.setProdAppNo(paser.generateAppNo(prodApplications));
			}
			prodApplications.setProdSrcNo(paser.getSrcNumber(prodApplications.getProdAppNo()));
		}*/
...
/*            if(!userSession.isCompany()){            	
            	      StreamedContent streamedContent = getTemplMBean().getReport(prodApplications.getId(), TemplateType.DOSSIER_RECIEPT);		
            	      prodApplicationsServiceMZ.createAckLetter(prodApplications, streamedContent);				
            }*/
            timeLine.setProdApplications(prodApplications);
