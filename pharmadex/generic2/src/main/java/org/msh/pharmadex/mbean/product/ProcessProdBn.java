...
public class ProcessProdBn implements Serializable {
	...
	public void silentSave() {
		facesContext = getCurrentInstance();
		try {			
		    prodApplicationsService.saveApplication(prodApplications, userSession.getLoggedINUserID());
	...
	public String registerProduct() {
		...
		if(prodApplications.getProdRegNo()==null||prodApplications.getProdRegNo().equals("")){
			String prodAppNo  = prodApplicationsService.generateAppNo(prodApplications);
			...
			prodApplications.setProdRegNo(prodApplicationsService.generateRegNo(prodAppNo, classificationNo));
			//03/10/17
	...
	/**
	 * old ProdApplication set State Archive
	 * create new ProdApplication in state SAVE
	 * @return
	 */
	public String createNewApplicationByRejProduct(){
		if(getProdApplications() != null){
			// send To archive current ProdApp
			getProdApplicationsService().sendToArchive(getProdApplications(), userSession.getLoggedInUserObj());
			
			// create new ProdApp
			ProdApplications newProdApplications = getProdApplicationsService().createNewProdApplications(getProdApplications(), userSession.getLoggedINUserID());
			
			getFlash().put("prodAppID", newProdApplications.getId());
			//Scrooge.setBeanParam("prodAppID", newProdApplications.getId());
			userSession.setProdAppID(newProdApplications.getId());
	...
	/**
	 * visible createNew Application button
	 * @return
	 */
	public boolean isVisibleCreateNewAppButton(){
		boolean res = false;
		try {
			if(getProdApplications().getRegState().equals(RegState.REJECTED)){
				if(userSession.isStaff()) // by Screener
					return true;

				// by Responsable of Applicant
				if(userSession.isResponsible(getProdApplications().getApplicant()))
					return true;

				if(userSession.isAgent(getProdApplications().getApplicant()))
					return true;
			}
			return res;
		} catch (Exception e) {
	...
	public List<ProdAppLetter> getLetters() {
		letters = prodApplicationsService.findAllLettersByProdApp(getProdApplications().getId(), userSession.isCompany());
		return letters;
	}
	...
	public boolean isApplicationFee(){
		if(getProdApplications() != null){
			...
			return true;//getProdApplications().getRegState().equals(RegState.SCREENING) || getProdApplications().getBankName() != null;
		}else{
			return false;
		}
	}
