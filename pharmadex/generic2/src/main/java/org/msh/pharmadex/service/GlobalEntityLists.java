...
public class GlobalEntityLists implements Serializable {
	...
	public List<PcCode> getPcCodes() {
		if(pcCodes==null)
			pcCodes= pcCodeService.getPcCodeList();
		return pcCodes;
	}

	public List<PcCode> completePcCodeNames(String query) {
	   return JsfUtils.completeSuggestions(query, getPcCodes());
	}
	 
	public PcCodeService getPcCodeService() {
		return pcCodeService;
	}

	public void setPcCodeService(PcCodeService pcCodeService) {
		this.pcCodeService = pcCodeService;
	}
