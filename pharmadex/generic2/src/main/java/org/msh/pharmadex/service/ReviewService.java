...
public class ReviewService implements Serializable {
	...
	public List<ReviewInfoTable> findRevFIRByUser(Long userID) {
		if (userID == null)
			return null;
		return customReviewDAO.findRevFIRByUser(userID);
	...
	public List<ReviewDetail> initReviewDetail(ReviewInfo reviewInfo, Long loggedInUser) {
		...
		if(reviewQuestions!=null && reviewQuestions.size()>0){
		...
		reviewInfo = reviewInfoDAO.findById(reviewInfo.getId());
	...
	public RetObject saveReviewInfo(ReviewInfo reviewInfo) {
		...
		Hibernate.initialize(ri.getProdApplications().getProduct());
		retObject.setObj(ri);
		retObject.setMsg("success");
		return retObject;
	}
	...
	/**
	 * Add review info to the database. Check conditions first
	 * @param reviewInfo
	 * @return
	 */
	...
	public ReviewDetailDAO getReviewDetailDAO() {
		return reviewDetailDAO;
	}

	public void setReviewDetailDAO(ReviewDetailDAO reviewDetailDAO) {
