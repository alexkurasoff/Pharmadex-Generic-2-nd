...
public class FileUploadController implements Serializable {
	...
	public void handleFileUpload(FileUploadEvent event) {
		...
		ProdApplications prodApplications = getReviewInfoBn().getReviewInfo().getProdApplications();
		try {
		...
		try {
			if (attachments == null){
				...
				if(processProdBn.getProdApplications() != null)
					prodAppId = processProdBn.getProdApplications().getId();
				else{
					if(getReviewInfoBn().getReviewInfo() != null && getReviewInfoBn().getReviewInfo().getProdApplications() != null)
						prodAppId = getReviewInfoBn().getReviewInfo().getProdApplications().getId();
				}
				if(prodAppId != null)
					attachments = (ArrayList<Attachment>) attachmentDAO.findByProdApplications_Id(prodAppId);
				else
