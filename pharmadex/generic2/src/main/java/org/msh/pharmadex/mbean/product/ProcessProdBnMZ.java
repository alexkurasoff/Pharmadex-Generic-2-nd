...
public class ProcessProdBnMZ implements Serializable {
	 ...
	 * @return
	 */
	public String registerProduct(ProdApplications prodApplications, TemplateType templateType) {
		facesContext = getCurrentInstance();
		try {
			
			String name = "";
			if(prodApplications!=null && prodApplications.getProduct()!=null){
			  name =  prodApplications.getProduct().getProdName().split(" ")[0] ;
			}
			name += "_registration"+".docx";
			
			 
			if(prodApplications.getRegState().equals(RegState.REGISTERED)){			
				StreamedContent streamedContent = getTemplMBean().getReport(name, prodApplications.getId(), templateType);
				prodApplicationsServiceMZ.createRegCert(prodApplications, getGestorDeCTRM(), isGeneric(), streamedContent);
			}else{
	...
	/**
	 * @param prodApplications id ProdApplications
	 * @param templateType -  TemplateType - REJECTION
	 * @return NMRC Rejection Letter
	 */
	public String rejectProduct(ProdApplications prodApplications, TemplateType templateType) {
		facesContext = getCurrentInstance();
		String name = "" ;
		if(prodApplications!=null && prodApplications.getProduct()!=null){
			name = prodApplications.getProduct().getProdName() ;
		}
		name += "_invoice"+ ".docx";
		
		if(prodApplications.getRegState().equals(RegState.REJECTED)){			
			StreamedContent streamedContent = getTemplMBean().getReport(name, getRejectSumm() , prodApplications.getId(), templateType);
			prodApplicationsServiceMZ.createRejectCert(prodApplications, streamedContent);
		}else{
